...
<div class="card">
    <div class="card-header bg-info d-flex justify-content-between align-items-center">
        ...
        <span class="badge badge-lg 
            @if($project->status == 'draft') badge-secondary
            @elseif($project->status == 'waiting_operasional') badge-warning
            @elseif($project->status == 'waiting_supporting') badge-info
            @else badge-success
            @endif">
            @if($project->status == 'draft') Draft
            @elseif($project->status == 'waiting_operasional') Menunggu Operasional
    ...
    <div class="card-body">
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <table class="table table-borderless mb-0">
                    <tr>
                        <td width="200"><strong>Skema</strong></td>
                        <td width="20">:</td>
                        <td>{{ $project->skema }}</td>
                    </tr>
                    <tr>
        ...
        @if($project->status != 'draft' && $project->status != 'waiting_operasional')
        <div class="card mb-4">
            <div class="card-header bg-secondary text-white">
        ...
        <div class="text-muted small">
            <p class="mb-1">Dibuat pada: {{ $project->created_at->format('d F Y, H:i') }}</p>
            @if($project->updated_at != $project->created_at)
            <p class="mb-0">Terakhir diubah: {{ $project->updated_at->format('d F Y, H:i') }}</p>
            @endif
        </div>
    </div>
</div>
